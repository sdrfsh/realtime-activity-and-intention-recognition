...
🎯 They run without a GPU, a dataset, a camera or the Keras model — the fast, no-data feedback loop for changes to that logic. The camera path (`LiveSession`) and the network are exercised through the CLI and the [demo](DEMO.md) instead.
